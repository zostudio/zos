...
import com.zos.generate.generate.util.AntPathMatcher;
import com.zos.generate.generate.util.BeanHelper;
import com.zos.generate.generate.util.FileHelper;
import com.zos.generate.generate.util.FreemarkerHelper;
...
public class Generator {
	...
	public String getOutRootDir() {
		// if(outRootDir == null) throw new IllegalStateException("'outRootDir' property
		// must be not null.");
		return outRootDir;
	}

	public void setProcessDir(String v) {
		if (v == null)
			throw new IllegalArgumentException("processDir must be not null");
		this.processDir = v;
	}
	...
	private class TemplateProcessor {
		...
		private void executeGenerate(File templateRootDir, Map templateModel, Map filePathModel, File srcFile)
			...
			if (isCopyBinaryFile && FileHelper.isBinaryFile(srcFile)) {
				...
				log.info("[copy binary file by extention] from:" + srcFile + " => "
		...
		private void executeDelete(File templateRootDir, Map templateModel, Map filePathModel, File srcFile)
			...
			// 只生成Java文件
			outputFilepath += ".java";
			log.info("[delete file] file:" + new File(gg.getOutRoot(), outputFilepath).getAbsolutePath());
		...
		private String proceeForOutputFilepath(Map filePathModel, String templateFile) throws IOException {
			...
			if ((testExpressionIndex = templateFile.indexOf('@')) != -1) {
				...
				if (expressionValue == null) {
					log.info("[not-generate] WARN: test expression is null by key:[" + testExpressionKey
							+ "] on template:[" + templateFile + "]");
					return null;
				}
				if (!"true".equals(String.valueOf(expressionValue))) {
					log.info("[not-generate]\t test expression '@" + testExpressionKey + "' is false,template:"
							+ templateFile);
					return null;
				}
			}
		...
		private void generateNewFileOrInsertIntoFile(String templateFile, String outputFilepath, Map templateModel)
			...
			if (absoluteOutputFilePath.exists() && !gg.isOverride()) {
				log.info("[not generate]\t by gg.isOverride()=false and outputFile exist:" + outputFilepath);
				return;
			}
	...
	static class GeneratorHelper {
		...
		public static Configuration newFreeMarkerConfiguration(List<File> templateRootDirs, String defaultEncoding,
			...
			conf.setAutoIncludes(availableAutoInclude);
			log.debug("set Freemarker.autoIncludes:" + availableAutoInclude + " for templateName:" + templateName
	...
	public static class GeneratorModel implements java.io.Serializable {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -691928254031421721L;
		@SuppressWarnings("rawtypes")
