package com.zos.generate.generate.util.sqlparse;
...
public class SqlParseHelper {
	...
	public static Set<NameWithAlias> getTableNamesByQuery(String sql) {
		...
		if (m.find()) {
			...
			if (from.indexOf(',') >= 0) {
				...
				// join的多表
				String removedFrom = StringHelper.removeMany(from.toLowerCase(), "inner", "full", "left", "right",
						"outer");
	...
	private static String getColumnNameByRightCondition(String sql, String column, String operator) {
		Pattern p = Pattern.compile("(\\w+)\\s*" + operator + "\\s*[:#$&]\\{?" + column + "[\\}#$]?",
	...
	/**
	 * 将sql从占位符转换为命名参数,如 select * from user where id =? ,将返回: select * from user
	 * where id = #id#
	 * 
	 * @param sql
	 * @param prefix
	...
	public static class NamedSqlConverter {
		...
		private String replaceInsertSql2NamedParameters(String sql) {
			...
			Pattern p = Pattern.compile("\\s*insert\\s+into.*\\((.*?)\\).*values.*?\\((.*)\\).*",
					Pattern.DOTALL | Pattern.CASE_INSENSITIVE);
			Matcher m = p.matcher(sql);
			if (m.find()) {
				...
				if (columns.length != values.length) {
					throw new IllegalArgumentException("insert 语句的插入列与值列数目不相等,sql:" + sql + " columns:"
							+ StringHelper.join(columns, ",") + " values:" + StringHelper.join(values, ","));
				}
		...
		private String replace2NamedParametersByOperator(String sql, String operator) {
			...
			while (m.find()) {
				...
				String paramName = StringHelper
						.uncapitalize(StringHelper.makeAllWordFirstLetterUpperCase(columnSqlName));
	...
	public static void setRandomParamsValueForPreparedStatement(String sql, PreparedStatement ps) throws SQLException {
		...
		for (int i = 1; i <= count; i++) {
			long random = new Random(System.currentTimeMillis() + startTimes++).nextInt() * 30
	...
	private static void warn(String sql, int i, SQLException error) {
		log.warn("error on set parametet index:" + i + " cause:" + error + " sql:" + sql);
	}
}
