package com.zos.generate.generate.provider.db.sql.model;

import java.util.Iterator;
import java.util.LinkedHashSet;

import com.zos.generate.generate.provider.db.sql.SqlFactory;
...
/**
 * 用于生成代码的SQL对象, 对应数据库的SQL语句 使用SqlFactory.parseSql()生成 <br />
 * 
 * SQL参数同时支持以下几种语法
 * 
 * <pre>
...
public class Sql {
	...
	String remarks;
	// many or one or paging
	String multiplicity = "many";
	// 是否分页查询
	boolean paging = false;
	// for ibatis and ibatis3
	String sqlmap;
	 ...
	 * resultClass可以为com.company.User的完全路径 示例:
	 * 
	 * <pre>
	 ...
	 * 
	 * <pre>
	 * 可以通过setParameterClass()自定义 没有自定义则: 如果是select查询,返回 operation+"Query"
	...
	/**
	 * 得到该sql方法相对应的操作名称,模板中的使用方式为: public List ${operation}(),示例值:
	 * findByUsername
	 * 
	 * @return
	 */
	...
	// sql =
	// StringHelper.replace(sql,":"+s.getParamName(),prefix+s.getParamName()+suffix);
	// }
	// return sql;
	// }
	/**
	 * sourceSql转换为在数据库实际执行的SQL, 示例:
	 * 
	 * <pre>
	...
	public String getIbatisSql() {
		return StringHelper.isBlank(ibatisSql) ? SqlParseHelper.convert2NamedParametersSql(getSql(), "#", "#")
				: ibatisSql;
	}

	public String getIbatis3Sql() {
		return StringHelper.isBlank(ibatis3Sql) ? SqlParseHelper.convert2NamedParametersSql(getSql(), "#{", "}")
				: ibatis3Sql;
	}
	...
	public String replaceWildcardWithColumnsSqlName(String sql) {
		if (isSelectSql() && SqlParseHelper.getSelect(sql).indexOf("*") >= 0
				&& SqlParseHelper.getSelect(sql).indexOf("count(") < 0) {
			return SqlParseHelper
					.getPrettySql("select " + joinColumnsSqlName() + " " + SqlParseHelper.removeSelect(sql));
		} else {
			return sql;
		}
	}
