...
import com.zos.generate.generate.util.ListHashtable;
...
public class ForeignKey {
	...
	private void initRelationship() {
		...
		List<Column> parentPrimaryKeys = parentTable.getPkColumns();
		List<Column> foreignPrimaryKeys = foreignTable.getPkColumns();
	...
	private boolean primaryKeyHasColumn(String aColumn) {
		...
		int numKeys = parentTable.getPkColumns().size();
		for (int i = 0; i < numKeys; i++) {
			Column sqlCol = parentTable.getPkColumns().get(i);
	...
	public static class ReferenceKey {
		...
		/**
		 * 解析foreignKey字符串,格式: fk_table_name(fk_column) 或者
		...
		public static ReferenceKey fromString(String foreignKey) {
			...
			String tableSqlName = foreignKey.substring(Math.max(foreignKey.lastIndexOf(".") + 1, 0),
					foreignKey.indexOf("("));
